...
- [x] `frontend/src/app/compass.tsx`가 화면 문구를 직접 문자열이 아닌 copy key로 쓰도록 변경
...
- [x] Travel POI 상세 시트의 카테고리, 교통 안내, 액션, 신고, 추천 문구 정리
- [x] Public Diary 시트의 제목/본문/날짜/Moments 이동 문구 정리
- [x] Shared Route Stop 시트의 스팟 타입/빈 메모/내 기록 남기기 문구 정리
- [x] Moments 화면의 공유 루트 카드 문구를 KO/EN/JA copy key 기반으로 전환
- [x] 공유 루트 상세 화면의 공개 전 확인, 추천, 댓글, 공유, 가져오기 문구 QA
- [x] Moments 화면의 언어 선택을 KO/EN/JA 세그먼트 컨트롤로 정리
